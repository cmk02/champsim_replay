...
WARMUP_ITERS="${WARMUP_ITERS:-20000000}"
MEASURE_ITERS="${MEASURE_ITERS:-10000000}"
...
run_cs_suite() {
  ...
  while IFS= read -r -d '' f; do
    ...
    echo " -> queued [$suite] $(basename "$f")"
           ...
           \( -name '*.champsimtrace.xz' -o -name '*phase*.trace.xz' -o -name '*.champsimtrace.gz' -o -name '*.trace.gz' \) \
           -print0 | sort -z)
