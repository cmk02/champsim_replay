...
cpu_stats operator-(cpu_stats lhs, cpu_stats rhs)
{
  ...
  for (int i=0; i<StallType::NUM_STALL_TYPE; i++){
    ...
    for(int j=0; j<StallCauseType::NUM_ROB_STALL_TYPE; j++){
      lhs.core_stall_cycles[i][j] -= rhs.core_stall_cycles[i][j];
      lhs.core_stall_counts[i][j] -= rhs.core_stall_counts[i][j];
    }
  }
